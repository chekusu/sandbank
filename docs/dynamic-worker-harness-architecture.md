...
Dynamic Workers execute bounded tool code through bindings. They do not provide a shell, container, VM filesystem, native package installation, browser automation, or long-running durable agent identity. Those workloads should be routed through Tool Use and the provider scheduler to a sandbox backend with the required capabilities, with Workspace sync preserving continuity across backends.
